...
`preJob.preInitDB.enabled` | 是否初始hzero_monitor数据库 | `true`
`preJob.preInitDB.datasource.url` | hzero_monitor数据库连接地址 | `jdbc:mysql://localhost:3306/hzero_minitor?useUnicode=true&characterEncoding=utf-8&useSSL=false&useInformationSchema=true&remarks=true`
`preJob.preInitDB.datasource.username` | hzero_monitor数据库用户名 | `choerodon`
`preJob.preInitDB.datasource.password` | hzero_monitor数据库密码 | `password`
...
`env.open.SPRING_DATASOURCE_URL` | 数据库连接地址 | `jdbc:mysql://localhost/hzero_monitor?useUnicode=true&characterEncoding=utf-8&useSSL=false&useInformationSchema=true&remarks=true`
