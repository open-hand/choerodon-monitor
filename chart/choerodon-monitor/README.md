...
To install the chart with the release name `choerodon-monitor`:

```console
$ helm repo add c7n https://openchart.choerodon.com.cn/choerodon/c7n
$ helm repo update
$ helm install choerodon-monitor c7n/choerodon-monitor
```
...
| containerPort.actuatorPort | int | `8261` | server management port |
| containerPort.serverPort | int | `8260` | server port |
...
| extraEnv.HZERO_AUDIT_ANNOTATION_ENABLE | bool | `true` |  |
| extraEnv.HZERO_AUDIT_API_ENABLE | bool | `true` |  |
| extraEnv.HZERO_AUDIT_OPERATION_ENABLE | bool | `true` |  |
| extraEnv.HZERO_RESOURCE_PATTERN | string | `"/v1/*,/hzero/*,/hmnt/v1/*,/hmnt/hzero/*"` |  |
| extraEnv.LOG_LEVEL | string | `"info"` |  |
| extraEnv.SPRING_DATASOURCE_PASSWORD | string | `"password"` |  |
| extraEnv.SPRING_DATASOURCE_URL | string | `"jdbc:mysql://localhost:3306/hzero_monitor?useUnicode=true&characterEncoding=utf-8&useSSL=false&useInformationSchema=true&remarks=true&serverTimezone=Asia/Shanghai"` |  |
...
| service.nodePort | object | `{"actuator":30119,"server":30118}` | Specify the nodePort value for the LoadBalancer and NodePort service types. |
| service.port | object | `{"actuator":8261,"server":8260}` | server Service port |
